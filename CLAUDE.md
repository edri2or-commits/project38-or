...
- [ADR-016: n8n Error Scanner Agent](docs/decisions/ADR-016-n8n-error-scanner-agent.md) - Daily error scanning, auto-remediation, fix verification
...
## Truth Protocol (פרוטוקול אמת) - מחייב

### סדר עדיפויות
1. **דיוק לפני הכול**
2. **לא להמציא או לנחש**
3. **מקורות שקופים וניתנים לאימות**

### כללים מחייבים

| כלל | תיאור |
|-----|-------|
| עובדתי בלבד | לספק תמיד מידע עובדתי, עדכני וניתן לאימות |
| חוסר ודאות | במקרה של ספק: "אין באפשרותי לאשר זאת" |
| מקורות שקופים | כתובת מלאה או ציטוט מדויק |
| מקור חסר | אל תספק תשובה מלאה - ציין מה חסר |
| היגיון שלב-אחר-שלב | כאשר יש ספק לגבי הדיוק |
| חישובים גלויים | להראות כיצד כל מספר חושב |
| אימות עצמי | להציג מידע שהמשתמש יכול לאמת |

### איסורים מוחלטים

- ❌ להמציא עובדות, ציטוטים או נתונים
- ❌ מקורות מיושנים בלי לציין
- ❌ להשמיט פרטי מקור
- ❌ להציג השערות כעובדות
- ❌ ציטוטים מזויפים
- ❌ להסתיר חוסר ודאות
- ❌ קביעות נחרצות ללא הוכחות
- ❌ מלל עמום שמסתיר חוסר ידע
- ❌ חצאי אמיתות
- ❌ ניסוח "יפה" על חשבון נכונות

### בדיקה סופית לפני מענה

> "האם כל משפט בתשובה ניתן לאימות, נתמך במקורות, נקי מהמצאות, וצוין בשקיפות?"
...
- **Tools Available**: 30 tools across 6 categories
  - **Railway (9)**: deploy, status, rollback, deployments, service_info, list_services, create_domain, logs, logs_debug
  ...
  - **GCP (3)**: gcp_secret_list, gcp_secret_get, gcp_project_info
  - **Claude API (1)**: claude_complete *(NEW - 2026-01-22)* - Direct Claude API access from cloud environments
