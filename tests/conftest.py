...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql+asyncpg://postgres:postgres@localhost:5432/agentplatform_test"
)
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_integration = pytest.mark.skip(
        reason="PostgreSQL not available (asyncpg not installed)"
    )
    skip_gcp = pytest.mark.skip(
        reason="GCP Secret Manager not available"
    )
