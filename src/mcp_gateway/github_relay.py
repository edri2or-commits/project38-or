...
from src.mcp_gateway.config import MCPGatewayConfig, load_config
    ...
    async def _execute_tool(
        ...
        try:
            if tool_name == "health_check":
                from src.mcp_gateway.tools.monitoring import health_check

                result = await health_check()
                return {"result": result}

            elif tool_name == "railway_status":
                from src.mcp_gateway.tools.railway import railway_status

                result = await railway_status()
                return {"result": result}

            elif tool_name == "railway_deploy":
                from src.mcp_gateway.tools.railway import railway_deploy

                result = await railway_deploy()
                return {"result": result}

            elif tool_name == "railway_rollback":
                from src.mcp_gateway.tools.railway import railway_rollback

                result = await railway_rollback()
                return {"result": result}

            elif tool_name == "n8n_list":
                from src.mcp_gateway.tools.n8n import n8n_list

                result = await n8n_list()
                return {"result": result}

            elif tool_name == "n8n_trigger":
                from src.mcp_gateway.tools.n8n import n8n_trigger
